...
- Откройте в браузере файл http://127.0.0.1:5000


## Как настроить

Для того чтобы отправлять реальные смс, нужно запускать сервер с параметром `--wet_run`, а также прописать `LOGIN` и `PASSWORD` от smsc.ru в файл `.env`. 
